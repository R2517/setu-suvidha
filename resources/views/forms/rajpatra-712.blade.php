@extends('layouts.app')
@section('title', 'राजपत्र ७/१२ शपथपत्र — SETU Suvidha')
@section('content')
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ showForm: false }">
    ...
    <div class="bg-gradient-to-br from-violet-600 to-purple-700 rounded-2xl p-8 text-white mb-8 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div class="flex items-center gap-4">
            ...
            <div><h1 class="text-2xl font-bold">राजपत्र ७/१२ शपथपत्र</h1><p class="text-white/80 text-sm mt-1">7/12 Affidavit | शुल्क: ₹{{ $pricing->price ?? '0' }}</p></div>
        </div>
        <button @click="showForm = !showForm" class="bg-white text-violet-600 font-semibold px-6 py-3 rounded-xl hover:bg-violet-50 transition flex items-center gap-2"><i data-lucide="plus" class="w-5 h-5"></i> फॉर्म भरा</button>
    </div>
    <div x-show="showForm" x-transition class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-8 mb-8">
        <form method="POST" action="/rajpatra-affidavit-712" class="space-y-5">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">जुने नाव (आडनाव)</label><input type="text" name="old_surname" class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition"></div>
                ...
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">नवीन नाव (आडनाव)</label><input type="text" name="new_surname" class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 transition"></div>
                ...
                <div class="md:col-span-2 flex items-center gap-2"><input type="checkbox" name="include_address" value="1" class="rounded border-gray-300 text-amber-600 focus:ring-amber-500"><label class="text-sm text-gray-700 dark:text-gray-300">पत्ता बदल समाविष्ट करा</label></div>
