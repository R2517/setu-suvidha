@extends('layouts.app')
@section('title', '{{ $formKey }} — SETU Suvidha')
@section('content')
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ showForm: false }">
    ...
    <div class="bg-gradient-to-br from-gray-600 to-gray-700 rounded-2xl p-8 text-white mb-8 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl bg-white/15 flex items-center justify-center"><i data-lucide="file-text" class="w-7 h-7"></i></div>
            <div><h1 class="text-2xl font-bold">{{ str_replace('_', ' ', ucfirst($formKey)) }}</h1><p class="text-white/80 text-sm mt-1">शुल्क: ₹{{ $pricing->price ?? '0' }}</p></div>
        </div>
        <button @click="showForm = !showForm" class="bg-white text-gray-600 font-semibold px-6 py-3 rounded-xl hover:bg-gray-50 transition flex items-center gap-2"><i data-lucide="plus" class="w-5 h-5"></i> फॉर्म भरा</button>
    </div>
