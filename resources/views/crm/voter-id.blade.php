@extends('layouts.app')
@section('title', 'Voter ID CRM — SETU Suvidha')
@section('content')
<div x-data="voterApp()" class="min-h-screen bg-gray-50 dark:bg-gray-950">
    <div class="bg-gradient-to-r from-sky-600 to-cyan-600 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-lg font-bold flex items-center gap-2"><i data-lucide="user-check" class="w-5 h-5"></i> Voter ID CRM</h1>
                    <p class="text-xs text-white/70">New, Correction, Transfer, Duplicate Applications</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            <a href="{{ route('voter-id') }}" class="block bg-white dark:bg-gray-900 rounded-xl border-2 {{ !request('status_filter') ? 'border-sky-500 shadow-md' : 'border-gray-200 dark:border-gray-800' }} p-4 text-center hover:shadow-lg transition">
                <i data-lucide="clipboard-list" class="w-6 h-6 mx-auto mb-1 text-sky-500"></i>
                <div class="text-2xl font-black text-sky-600">{{ $allCount }}</div>
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">ALL</div>
            </a>
            <a href="{{ route('voter-id', ['status_filter' => 'pending']) }}" class="block bg-white dark:bg-gray-900 rounded-xl border-2 {{ request('status_filter') === 'pending' ? 'border-amber-500 shadow-md' : 'border-gray-200 dark:border-gray-800' }} p-4 text-center hover:shadow-lg transition">
            ...
            <a href="{{ route('voter-id', ['status_filter' => 'processing']) }}" class="block bg-white dark:bg-gray-900 rounded-xl border-2 {{ request('status_filter') === 'processing' ? 'border-blue-500 shadow-md' : 'border-gray-200 dark:border-gray-800' }} p-4 text-center hover:shadow-lg transition">
            ...
            <a href="{{ route('voter-id', ['status_filter' => 'completed']) }}" class="block bg-white dark:bg-gray-900 rounded-xl border-2 {{ request('status_filter') === 'completed' ? 'border-green-500 shadow-md' : 'border-gray-200 dark:border-gray-800' }} p-4 text-center hover:shadow-lg transition">
            ...
            <a href="{{ route('voter-id', ['status_filter' => 'rejected']) }}" class="block bg-white dark:bg-gray-900 rounded-xl border-2 {{ request('status_filter') === 'rejected' ? 'border-red-500 shadow-md' : 'border-gray-200 dark:border-gray-800' }} p-4 text-center hover:shadow-lg transition">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6">
            <button @click="showForm = !showForm" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-sky-500 to-cyan-500 shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all">
                <i data-lucide="plus" class="w-4 h-4"></i> New Application
            </button>
            <button @click="showFilters = !showFilters" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:border-sky-400 transition">
                <i data-lucide="filter" class="w-4 h-4"></i> Filter
            </button>
            <form method="GET" action="{{ route('voter-id') }}" class="flex-1 max-w-sm ml-auto">
                <div class="relative">
                    <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"></i>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name / mobile / aadhar..."
                        class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:ring-2 focus:ring-sky-500 transition">
                </div>
            </form>
        </div>
        ...
        <div x-show="showFilters" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full"
            ...
            <div class="p-5">
                ...
                <form method="GET" action="{{ route('voter-id') }}">
                    <div class="mb-5">
                        <p class="text-xs font-bold text-gray-500 uppercase mb-2">Application Type</p>
                        @foreach(['new' => 'New', 'correction' => 'Correction', 'transfer' => 'Transfer', 'duplicate' => 'Duplicate'] as $val => $label)
                        <label class="flex items-center gap-2 py-1 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                            <input type="checkbox" name="app_type[]" value="{{ $val }}" {{ in_array($val, (array) request('app_type', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-sky-600 focus:ring-sky-500"> {{ $label }}
                        </label>
                        @endforeach
                    </div>
                    <div class="mb-5">
                        ...
                        <label class="flex items-center gap-2 py-1 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                            <input type="checkbox" name="pay_status[]" value="{{ $val }}" {{ in_array($val, (array) request('pay_status', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-sky-600 focus:ring-sky-500"> {{ $label }}
                        </label>
                        @endforeach
                    </div>
                    <div class="mb-5">
                        ...
                        <label class="flex items-center gap-2 py-1 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                            <input type="checkbox" name="pay_mode[]" value="{{ $val }}" {{ in_array($val, (array) request('pay_mode', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-sky-600 focus:ring-sky-500"> {{ $label }}
                        </label>
                        @endforeach
                    </div>
                    <div class="flex gap-2">
                        <button type="submit" class="flex-1 btn-primary text-xs !py-2">Apply</button>
                        <a href="{{ route('voter-id') }}" class="flex-1 text-center py-2 text-xs font-medium text-gray-600 border border-gray-300 rounded-xl hover:bg-gray-50">Clear</a>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="showForm" x-transition class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-6">
            <h3 class="text-base font-bold text-gray-900 dark:text-white mb-5 flex items-center gap-2">New Voter ID Application</h3>
            <form method="POST" action="{{ route('voter-id.store') }}">
                @csrf
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-sky-500 to-cyan-500 shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all">
                    Save Application
                </button>
            </form>
        </div>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition" x-data="{ editing: false }">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-1">
                                    <button @click="editing = !editing" class="p-1.5 rounded-lg text-sky-600 hover:bg-sky-50 dark:hover:bg-sky-900/20 transition" title="Edit"><i data-lucide="pencil" class="w-4 h-4"></i></button>
                                    <form method="POST" action="{{ route('voter-id.destroy', $app->id) }}" onsubmit="return confirm('Delete?')">
                                        @csrf @method('DELETE')
